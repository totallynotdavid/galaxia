---
title: Diseño
description: Herramientas de diseño y recursos para mejorar la estética de tus proyectos.
---

[EN] [The Bézier Game](https://bezier.method.ac/) es un videojuego para aprender a usar la pluma

## Animaciones

| Nombre | Características | Enlace |
| --- | --- | --- |
| Finsweet LottieFlow | Animaciones gratuitas y simples para iconos. | [Visitar](https://www.finsweet.com/lottieflow/) |
| LottieFiles | Animaciones de objetos usando CSS. | [Visitar](https://lottiefiles.com/) |
| Brush.ninja | Creación de animaciones frame a frame y GIFs. | [Visitar](https://brush.ninja/) |

## Iconos

| Nombre | Características | Enlace |
| --- | --- | --- |
| Unicons | Iconos de línea, versión gratuita disponible. | [Visitar](https://iconscout.com/unicons) |

## Ilustraciones

| Nombre | Características | Enlace |
| --- | --- | --- |
| Storyset | Ilustraciones gratuitas. | [Visitar](https://storyset.com/) |
| Icons8 Illustrations | Vectores 3D e ilustraciones gratuitas. | [Visitar](https://icons8.com/illustrations) |
| GetIllustrations | Ideas de ilustraciones (premium). | [Visitar](https://www.getillustrations.com/) |

## Video

| Nombre | Características | Enlace |
| --- | --- | --- |
| Visme Templates | Plantillas para videos, presentaciones, etc. | [Visitar](https://www.visme.co/templates/videos/) |

## Tipografías

| Nombre | Características | Enlace |
| --- | --- | --- |
| Fontkey | Explorador de tipografías. | [Visitar](https://www.fontkey.design/) |
| FontSpark | Generador de tipografías para proyectos. | [Visitar](https://fontspark.app/) |
| Nodesign | Recursos de diseño sin necesidad de conocimientos técnicos. | [Visitar](https://nodesign.dev/) |

## Color

| Nombre | Características | Enlace |
| --- | --- | --- |
| Nippon Colors | Paleta de colores japoneses tradicionales. | [Visitar](https://nipponcolors.com/) |
